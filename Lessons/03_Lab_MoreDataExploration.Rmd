...
* **Data Frames** 2 dimensional structure that is more general than a matrix. Columns can have different modes (e.g., numeric and factor). When we import csv files into the R workspace, they will enter as data frames. Columns are vectors, of all the same data type.
...
> Answer: Vectors are listed as a value, data frame and matrix are data.
...
TemperatureFHigh <-c(58,41,43,45,35,42,49,52,56,66,59,49,49,48,50)
TemperatureFLow<-c(31,19,25,27,18,23,25,29,43,49,36,32,30,31,36)
...
```{r}
f_to_c <- function(x){
  CelsiusConvert<-((x-32)*(5/9))
  return(CelsiusConvert)
}
TemperatureCHigh <-f_to_c(TemperatureFHighs)

TemperatureCHigh

TemperatureCLow <-f_to_c(TemperatureFLows)
TemperatureFLows
...
```{r}
Temp_df <-cbind(TemperatureFHigh,TemperatureFLow,TemperatureCHigh,TemperatureCLow)

colnames(Temp_df)<-c("Fahrenheit High","Fahrenheit Low","Celsius High","Celsius Low")
```
