...
> Answer: null- the difference between our sample and 50 is zero, alternative- the difference is not 0
...
DensityPlot <-ggplot(EPAair.subsample, aes(x=Ozone, fill = Year))+
         geom_density(alpha = 0.5) + 
         labs(y = 'Density', x = 'Ozone AQI Value', title = "Density Plot: Ozone by Year")+
  scale_fill_manual(values= c('blue','yellow'))

print(DensityPlot)

#2
Ozoneboxplot <- ggplot(EPAair, aes(y = Ozone, x = Year)) +
  geom_boxplot()+
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 4, label =c("a", "b"), color = 'red')+
  labs(x = "Year", y = "Ozone AQI Values")
print(Ozoneboxplot)

#3
OzoneViolinPlot <- ggplot(EPAair, aes(x = Year, y = Ozone))+
  geom_violin(draw_quantiles = 0.5) +
  stat_summary(geom ="text", fun.y = max, vjust = -1, size = 4,  label = c("a", "b") )
print(OzoneViolinPlot)
...
Ozone.regression <- lm(EPAair$Ozone ~ EPAair$PM2.5)
summary(Ozone.regression)

#pvalue is so low that we can reject the null hypothesis, which means that there very likely is a relationship between our two variables. 

#Exercise 4: Build a scatterplot. Add a line and standard error for the linear regression.

Ozone.Regression.Plot <- ggplot(EPAair, aes(y=PM2.5, x=Ozone)) +
  geom_point() +
  geom_smooth(method = 'lm', se = TRUE, color = 'red')
plot(Ozone.Regression.Plot)
