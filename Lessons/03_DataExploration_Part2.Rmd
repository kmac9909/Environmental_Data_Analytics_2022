...
Histograms are useful for plotting numerical data
...
```{r, fig.height = 3, fig.width = 4}
#Plotting discharge max in histogram
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.max))

#Histogram plot of discharge mean, increasing # of bins to get smoother data
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#Histogram plot of discharge mean with only 20 bins, creating chunky data whereas default is 30
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#Zooming on on where your data is located
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#histogram of the gage height
ggplot(USGS.flow.data) +
...
> Answer: 
-bar plot is useful for analyzing categorical data
-histogram is useful for numerical data
-frequnecy polygon is useful for comaring multiple sets of categorical data
-box and whisker helps you find outliers and skews in numerical data
-violin plots help you visualize density (where the most numerical data sets are located)
-scatterplots help you visualize relationships between 2 sets of numberical data

## Ending discussion

What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: Discharge increases as gage height increases, discharge was not always measured. Discharge is distributed in a upward U shape, the most # of cases had 0 or very small values


How can multiple options for data exploration inform our understanding of our data?

> Answer: They can reveal information that may have been previously missed, for instance, we were revealed that between the years 1970-1990, no data was pulled on discharge. 
...
> Answer: there is a direct relationship between the gage height and amount of discharge
