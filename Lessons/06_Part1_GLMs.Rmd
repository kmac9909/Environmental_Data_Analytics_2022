...
~~Lesson notes from 2/14~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
if p <0.05 we can reject the null hypothesis
if p> 0.05 we cannot reject the null hypothesis
-> p values indicate whether the relationship between X and Y is significant, p is the probability that the null is true (if p is super small, you reject the null)

-regression is a technique for fitting a line to set of data points
-simple linear delineation allows you to find a linear relationship among variable X and Y
-Standard error is the measure of the distance between points around a regression line (you can a small se)
  -correlation (+,-, no correlation)
-regression coefficient i the slope of the regression line
-fitted values are estimates of Y obtained from the prediction
  --> residuals are the difference between predicted (above) and observed
-R^2 is the measure of % of variability that is explained by independent variable
  -high R^2 means that the variability is represented
-Residuals:
  -check to be sure unbiased estimates
-you can transform the data (log)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
--> sorting data along error
--> shows the distribution of residuals, you want this to be a straight line
...
Given the distribution of irradiance values, we don't have a linear relationship between x and y in this case. Let's try log-transforming the irradiance values. Note we also removing the observations that seems to be a an outlier.

```{r, fig.height = 3, fig.width = 4}
#cannot run log analysis is any of the values = 0, we need to filter this out
...
The most correlated is narrow and directed to the side. Helps you find which variables are not correlated.
