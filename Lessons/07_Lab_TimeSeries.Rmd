...
Rainfall_time_series <-ggplot(Rainfall_BR_processed, aes(x = Date, y = Rainfall))+
  geom_line()+
  geom_smooth(method = "lm")
print(Rainfall_time_series)
  
Temp_time_series <- ggplot(Temp_BR_processed, aes(x=Date, y = Temperature_C))+
  geom_line()+
  geom_smooth(method = "lm")
...
Rainfall_Decomposed <- stl(BR_rain_ts, s.window = "periodic")
plot(Rainfall_Decomposed)

Temperature_Decomposed <- stl(BR_Temp_ts, s.window = "periodic")
...
Temp_Components <- as.data.frame(Temperature_Decomposed$time.series[,1:3])
