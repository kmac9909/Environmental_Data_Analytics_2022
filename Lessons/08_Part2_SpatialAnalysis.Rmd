...
mapView(counties_sf, zcol = "AWATER") #or try "ALAND"
```
...
ggplot() +
  ...
  geom_sf(data = huc8_sf_subset, fill = 'purple')
...
wind_energy_huc8 <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c("HUC_12" = 'factor')) %>%
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #1,8 is where to grab rows, columns from
  group_by(HUC_8) %>%
...
#View the outputs
mapview(huc8_sf_wind, zcol = 'Wind energy', col.regions = brewer.pal(2,'RdBu'))+
  mapview()

ggplot(data = huc8_sf_wind) +
...
#st intersects creates the intersection of all features in X that intersect w/ counties of interest (triangle area). X = ., y = triangle_co
triangle_HUCs <- huc8_sf_utm %>%
  filter(st_intersects(x = ., y = tirangle_co, sparse = F))
...
#cookie dough = huc data, cookie cutter = triangle countiesf
